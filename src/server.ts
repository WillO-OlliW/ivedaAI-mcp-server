...
import { z } from "zod";
import { type SwaggerContext, tagToToolName, resolveRef, schemaDefinitions, schemaRef, type Operation } from "./swagger.js";
...
import { type AccessPolicy, refusalReason, allowedOperations, isReadSafe } from "./accessPolicy.js";
...
export function createIvedaServer(
  ctx: SwaggerContext,
  tokenManager: TokenManager,
  ACCESS_POLICY: AccessPolicy,
  options: { enforceLossyUpdateGuard?: boolean; log?: (...args: unknown[]) => void } = {},
) {
   ...
   */
  const ENFORCE_LOSSY_UPDATE_GUARD = options.enforceLossyUpdateGuard !== false;
  ...
  for (const group of ctx.tags) {
    ...
    if (seenNames.has(toolName)) {
      log(`[ivedaai-mcp-server] duplicate tool name "${toolName}" for tag "${group.tag}", skipping`);
      continue;
    }
  ...
  return { server, resourceToolCount: seenNames.size, handWrittenTools: registeredHandWrittenTools };
}
